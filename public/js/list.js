...
function loadList() {
  ...
  fetch("/api/list").then(function (res) { return res.json(); }).then(function (data) {
    ...
    const lifecycle = ["Needed", "In Cart", "Purchased", "Consumed"];
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      html += '<div class="grocery-item ' + statusClass + '">' +
        ...
        '<span class="status-tag ' + statusClass + '" onclick="toggleStatus(' + item.id + ",'" + nextStatus + '\')" style="cursor:pointer;">' + item.status + '</span>' +
        '<div class="item-actions">' +
        '<button class="btn btn-secondary btn-sm" onclick="toggleStatus(' + item.id + ",'" + nextStatus + "')\">" + nextStatus + '</button>' +
        '<button class="btn btn-danger btn-sm" onclick="deleteItem(' + item.id + ')">Remove</button></div></div>';
...
function downloadList() {
  ...
  }).catch(function () { alert("Failed to download list"); });
}